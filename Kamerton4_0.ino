...
unsigned int volume_porog_D       = 30;                     // Величина порога при проверке исправности FrontL,FrontR
float voltage ;
...
byte prer_Kmerton_On = 0;                           // Флаг разрешение прерывания Камертон
byte prer_Kmerton_Run = 0;                          // Флаг разрешение прерывания Камертон
...
void Stop_Kamerton ()                                     // Не приходит информация с Камертона - регистры обнулить
  {
...
/*
		...
		delay(1000);
		  ...
		  // Проверить исправность канала динамиков
...
//	sence_all_off();
	...
	prer_Kmerton_On = 1;                             // Разрешить прерывания на камертон
	mcp_Analog.digitalWrite(Front_led_Red, LOW); 
